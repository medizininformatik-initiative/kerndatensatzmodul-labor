...
* insert Translation(interpretation ^binding.description, en-US, [[Restricted selection from HL7 v3 ObservationInterpretation. FHIR binds this element extensibly to the complete Observation Interpretation Codes value set; the module narrows it to the concepts meaningful in a laboratory context. Local codings such as -- - N + ++ or L N H can be mapped onto these concepts. As the binding remains extensible, the abnormal codes HH, LL and AA are additionally permitted, for example for values beyond the critical notification limit.]])
* note MS
  * ^short = "Hinweis"
